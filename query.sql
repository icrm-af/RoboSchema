...
select schema_name(tab.schema_id) as schema_name,
     ...
     t.name as data_type,
     t.name + case
          when t.is_user_defined = 0 then isnull(
               '(' + case
                    when t.name in (
                         'binary',
                         'char',
                         ...
                         when -1 then 'MAX'
                              ...
                              when t.name in (
                                   ...
                                   col.max_length / 2 as varchar(4)
                              )
                              else cast(
                                   col.max_length as varchar(4)
                              )
                         end
                    end
                    when t.name in (
                    ...
                    when t.name in ('decimal', 'numeric') then cast(col.precision as varchar(4)) + ', ' + cast(col.scale as varchar(4))
               end + ')',
               ''
          )
          else ':' + (
               select c_t.name + isnull(
                         '(' + case
                              when c_t.name in (
                                   ...
                                   when -1 then 'MAX'
                                        ...
                                        when t.name in (
                                             ...
                                             c.max_length / 2 as varchar(4)
                                        )
                                        else cast(
                                             c.max_length as varchar(4)
                                        )
                                   end
                              end
                              when c_t.name in (
                              ...
                              when c_t.name in ('decimal', 'numeric') then cast(c.precision as varchar(4)) + ', ' + cast(c.scale as varchar(4))
                         end + ')',
                         ''
                    )
